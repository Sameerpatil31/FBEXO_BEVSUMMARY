...
class YearlyFinancialData(BaseModel):
    year: int = Field(..., description="The fiscal or calendar year of the financial data, represented as an integer (e.g., 2023, 2022, etc.). Identify the most recent and prior years, typically the last 3 years available.")
    ...
    Net_Profit: int = Field(0, description="Net profit or net income after all expenses, taxes, and interest (may be labeled as 'Net income', 'Net profit', 'Profit after tax', etc.) reported in millions. This is the bottom line of the income statement.")
...
class PDFCompanyExtractor:
    ...
    def extract_text_from_pdf_url_temp(self, pdf_url: str) -> str:
        ...
        :param pdf_url: Public URL to the PDF file (e.g., S3 bucket URL).
    ...
    """),
        response_model=self.FinancialMetrics,
        use_json_mode=True,
        context={"financial_doc":financial_doc},
        add_context=True,
        instructions=dedent("""
        ...
        #     # isolate JSON braces if extra text present
        #     first = cleaned.find('{')
        #     last = cleaned.rfind('}')
        #     if first != -1 and last != -1:
        #         cleaned = cleaned[first:last+1]
        #     try:
        #         fm_instance = FinancialMetrics.model_validate_json(cleaned)
        #     except Exception:
        #         data = json.loads(cleaned)
        #         fm_instance = FinancialMetrics.model_validate(data)
        # elif isinstance(content, dict):
        #     fm_instance = FinancialMetrics.model_validate(content)
        # else:
        #     raise ValueError(f"Unsupported FinancialMetrics content type: {type(content)}")

        # return fm_instance.model_dump_json(indent=2)

        r2 = self.FinancialMetrics.model_dump_json(a2.content, indent=2)
    ...
    def extract_risk_factors(self, financial_doc: str) -> RunResponse:
        ...
        You are RiskFactorsExtractor, an AI agent specialized in parsing and extracting risk factors from lengthy financial documents such as annual reports, 10-K filings, or risk factor sections, which may span 80-90 pages or more.
            ...
            - market_cyclicality: Assessed market cyclicality as 'low', 'medium', or 'high' based on document descriptions (e.g., cyclical nature of automotive industry); 'medium' if unclear.\n
                            ...
                            "risk_factors":"{
  "high_customer_concentration": false,
  ...
  "yearly_data": [
    {
      "year": "2023",
      "net_income": 10.127,
      "adjustments_for_non_cash_items": 7987.0,
      "changes_in_working_capital": 3874.0,
      "cash_from_operating_activities": 20828.0,
      "cash_from_investing_activities": -14663.0,
      "cash_from_financing_activities": -6353.0,
      "net_cash_flow": -618.0,
      "beginning_cash_balance": 21948.0,
      "ending_cash_balance": 21930.0
    },
    {
      "year": "2022",
      "net_income": 9.934,
      "adjustments_for_non_cash_items": 8765.0,
      "changes_in_working_capital": -2997.0,
      "cash_from_operating_activities": 19094.0,
      "cash_from_investing_activities": -17882.0,
      "cash_from_financing_activities": 383.0,
      "net_cash_flow": -1405.0,
      "beginning_cash_balance": 23542.0,
      "ending_cash_balance": 22187.0
    }
  ]
}"
                            """))
    ...
    def extract_all(self, file_path_or_url: str) -> dict:
        ...
        :param financial_doc: Extracted text from the PDF.
        :return: Dictionary containing all extracted financial data.
        """
        # financial_doc  = self.extract_text_from_pdf(pdf_path=file_path_)
        ...
        # Return the final report containing all extracted data
        return final_report
